...
public class JUnit_SuiteScriptsOptimal extends JUnit_Test
{
		...
		public JUnit_SuiteScriptsOptimal(MemoryManager memorymanager, ScriptManager scriptmanager, GameState<RobotVrai> real_state, Read_Ini config, Log log)
		{
		...
		int temps = 0;
		Branche branche;
		NoteScriptMetaversion bäst;
	
		
		public void force_brute_branche(ArrayList<NoteScriptMetaversion> dejaFait)
		{
			if(dejaFait.size() == 0)
			{
				temps = 0;
				branche = evaluate(dejaFait);
				temps += branche.dureeScript;
				
				bäst.note = branche.note;
				bäst.script = branche.script;
				bäst.metaversion = branche.metaversion;
				dejaFait.add(bäst);
				force_brute_branche(dejaFait);
		...
		public void test_force_brute_branche()
		{
			ArrayList<NoteScriptMetaversion> laSuite = null;
			ArrayList<String> lesScripts = scriptmanager.getNomsScripts();
			
			force_brute_branche(laSuite);
			
			for(int i = 0; i < laSuite.size();i++)
			{
				System.out.println("Le script est : "+laSuite.get(i).script.toString());
				System.out.println("La métaversion en question est : "+laSuite.get(i).metaversion);
				System.out.println();
		...
		public Branche evaluate(ArrayList<NoteScriptMetaversion> errorList)
		{
			/*
