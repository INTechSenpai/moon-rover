...
Motor::Motor(Side s) : side(s), maxPWM(170)
{
...
void Motor::initPWM(){
	 ...
	 * TIM8 CH1 (PC6 = moteur droit) et TIM8 CH2 (PC7 = moteur gauche)
	 */
	...
	/* -----------------------------------------------------------------------
	 TIM8 Configuration: génère 2 PWM à deux rapports cycliques différents (un timer a 4 canaux,
...
void Motor::run(int16_t pwm){
	...
	if (pwm >= 0) {
		...
		if (side == Side::LEFT) {
			TIM8->CCR1 = MIN(pwm, maxPWM);
		} else {
			TIM8->CCR2 = MIN(pwm, maxPWM);
		}

	} else {
		setDirection(Direction::BACKWARD);
		if (side == Side::LEFT) {
			TIM8->CCR1 = MIN(-pwm, maxPWM);//Pourquoi cette formule ?? ça devrait être : -MIN(pwm, maxPWM)
		} else {
			TIM8->CCR2 = MIN(-pwm, maxPWM);
		}
	}
