...
bool inline verifieChecksum(unsigned char* m, uint8_t longueur)
{
	...
	return (~c & 0xFF) == lu; // c est cast� en int avant de faire ~, donc il faut v�rifier uniquement l'octet de poids faible
}
...
//		serial_rb.write((char*)&longueur);
		...
		// �tant donn� que la source peut �tre la m�me que la destination, on utilise memmove � la place de memcpy
...
void inline sendCapteur(uint16_t x, uint16_t y, uint16_t orientation, uint8_t courbure, bool marcheAvant, int16_t vitesseLineaireReelle, int16_t vitesseRotationReelle, uint16_t c[])
{
